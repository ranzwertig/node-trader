(function() {
  var Cache, Crawler, Endpoint, FinanzennetEndpoint,
  ...
  */
  ...
  FinanzennetEndpoint = (function(_super) {
    ...
    function FinanzennetEndpoint(cacheResults) {
      this.cacheResults = cacheResults != null ? cacheResults : true;
      this.crawlIndex = __bind(this.crawlIndex, this);

      this.searchIndex = __bind(this.searchIndex, this);
    ...
    */
    ...
    FinanzennetEndpoint.prototype.searchIndex = function(name, cb) {
      ...
      fromCache = false;
      if (this.cache) {
        this.cache.get(name, function(err, value) {
          if (!err) {
            fromCache = true;
            cb(null, value);
            return cb = function() {};
        ...
        this.crawler.queue([
          {
            ...
            callback: function(error, result, $) {
              ...
              if (_this._isValidIndexUrl(result.request.href)) {
                return _this.crawlIndex(result.request.href, cb);
              } else {
      ...
      this.crawler.queue([
        {
          ...
          callback: function(error, result, $) {
            ...
            if (_this._isValidEquityUrl(result.request.href)) {
              return _this.crawlEquity(result.request.href, stockMarket, cb);
            } else {
              url = $('.main table tr').eq(1).find('a').attr('href');
              if (!url) {
                return cb(null, null);
    ...
    */
    ...
    FinanzennetEndpoint.prototype.getEquityUrlsByIndex = function(indexName, cb) {
      var _this = this;
      this.searchIndex(indexName, function(err, index) {
        if (err) {
          cb(err, null);
          return;
        }
      ...
      this.crawler.queue([
        {
          uri: url,
          callback: function(error, result, $) {
            var a, equity, finanzennetId, finanzennetIdRegexp, matches, now, priceStr, _i, _len, _ref;
            if (error) {
              cb(new Error('Could not load finanzen.net!'), null);
              return;
            }
            ...
            return _this.crawler.queue([
              {
                ...
                callback: function(err, result, $) {
                  ...
                  if (_this.cache) {
                    _this.cache.set(index.name, index, function(err) {});
                  }
